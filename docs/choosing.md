...
!!! abstract "The one idea to take away"
    **dbt and Spark are two interchangeable, self-contained pipelines** — each *ingests and
    ...
    loader** — like `dbt seed`, it lands the raw CSVs in `lakehouse_demo_ingest` and *stops at raw*.
...
| **Output schema** | `lakehouse_demo_raw/bronze/silver/gold` | `spark_demo_bronze/silver/gold` | `lakehouse_demo_ingest` (raw) |
...
=== "Use cpdctl"
    ...
    - Then you still run **dbt or Spark** over `lakehouse_demo_ingest` to build bronze/silver/gold.
...
```text
dbt:   seeds/ CSV → lakehouse_demo_raw → bronze → silver → gold
...
```text
cpdctl: CSV → lakehouse_demo_ingest (raw)
...
The worked post-action examples (a dbt model and a Spark read over `lakehouse_demo_ingest`) are in
