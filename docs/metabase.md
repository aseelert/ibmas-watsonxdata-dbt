...
![Metabase home page](assets/images/screenshots/metabase-home.png)
...
!!! warning "Metabase is OPTIONAL in this demo"
    Nothing in the medallion pipeline needs Metabase. The [dbt](dbt-demo.md) and [Spark](spark-demo.md) paths build every `raw → bronze → silver → gold` table on their own, and you can query the results with `python scripts/05_query_gold.py` or the [SQL comparison demo](sql-demo.md). Metabase is here purely so you can *see and chart* those gold tables in a friendly browser UI. Skip it if you only want the pipeline; run it if you want a BI front end.
