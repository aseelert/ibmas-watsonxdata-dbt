...
(scripts/00b_get_token.py, scripts/03b_submit_spark_application.py, scripts/01_bootstrap_watsonxdata.py,
scripts/05_query_gold.py) so we never duplicate that logic across DAGs.
...
# ---------------------------------------------------------------------------
# CPD username / API key (mirrors scripts/03b_submit_spark_application.py)
# ---------------------------------------------------------------------------
...
def mint_bearer_token() -> str:
    ...
    Same call as scripts/00b_get_token.py — re-minting each run keeps the scheduled
...
def zen_api_key() -> str:
    ...
    Mirrors _zen_auth_string() in scripts/03b_submit_spark_application.py, which
...
def spark_applications_endpoint() -> str:
    # Default mirrors scripts/03b_submit_spark_application.py and .env.example (api/v3).
...
# ---------------------------------------------------------------------------
# Presto connection (mirrors scripts/01_bootstrap_watsonxdata.py / scripts/05_query_gold.py)
# ---------------------------------------------------------------------------
