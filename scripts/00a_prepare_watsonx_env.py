...
#
#  Location  : scripts/00a_prepare_watsonx_env.py
...
USAGE
    python scripts/00a_prepare_watsonx_env.py                  # normal run
    python scripts/00a_prepare_watsonx_env.py --oc-login       # oc login first, then run
    python scripts/00a_prepare_watsonx_env.py --dry-run        # show diff, write nothing
    python scripts/00a_prepare_watsonx_env.py --overwrite      # replace existing non-secret values
    python scripts/00a_prepare_watsonx_env.py --fetch-tokens   # also derive API key + bearer token
    python scripts/00a_prepare_watsonx_env.py --oc-login --fetch-tokens  # fully automated
    python scripts/00a_prepare_watsonx_env.py --presto-json /other/export.json
    python scripts/00a_prepare_watsonx_env.py --spark-json watsonx_data/spark_details.json
...
def _write_env(
    ...
    lines: list[str] = list(preamble) if preamble else [
        ...
        "# Re-run:  python3 scripts/00a_prepare_watsonx_env.py --oc-login --fetch-tokens",
        "",
    ]
    ...
    if missing_secrets:
        ...
        for key in missing_secrets:
            if key == "WXD_API_KEY":
                ...
                print(f"   {key}  ← run:  python scripts/00b_get_token.py --export")
            else:
                print(f"   {key}")
...
def _bootstrap_wizard(env_path: Path) -> None:
    ...
    env_path.write_text(
        ...
        "# Run:  python3 scripts/00a_prepare_watsonx_env.py --oc-login --fetch-tokens\n"
    ...
    print(f"      python3 scripts/00a_prepare_watsonx_env.py --oc-login --fetch-tokens\n")
