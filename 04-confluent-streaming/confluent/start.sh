...
CONFLUENT_DIR="${REPO_ROOT}/04-confluent-streaming/confluent"
...
GOLD_SPARK_SCRIPT="04-confluent-streaming/confluent/scripts/submit_confluent_gold.py"
GOLD_DATASTAGE_SCRIPT="04-confluent-streaming/confluent/scripts/create_datastage_flow.py"
...
build_image() {
  ...
  if docker image inspect wxd-flink:1.20 > /dev/null 2>&1; then
    ...
    run docker build -t wxd-flink:1.20 -f "${CONFLUENT_DIR}/flink/Dockerfile" "${CONFLUENT_DIR}/flink/"
...
seed_kafka() {
  ...
  run "$VENV_PYTHON" "${CONFLUENT_DIR}/scripts/ingest_csv_to_kafka.py" \
    --bootstrap-servers "$KAFKA_BOOTSTRAP" \
    --csv-dir 01-dbt/seeds/
